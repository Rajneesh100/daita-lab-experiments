...
def main():
    ...
    print(f"Found {len(emails_json)} order emails in specified range")
    
    print(f"\n📧 CLEAN EMAIL JSON STRUCTURE:")
    print("=" * 60)
    
    import json
    
    for i, email_json in enumerate(emails_json, 1):  
        print(f"\n📨 Email {i}:")
        print(f"From: {email_json['from']}")
        print(f"To: {email_json['to']}")
        print(f"Subject: {email_json['subject']}")
        print(f"Date: {email_json['date_time']}")
        print(f"Content: {email_json['content'][:150]}{'...' if len(email_json['content']) > 150 else ''}")
        print(f"PDF Attachments ({len(email_json['attachments'])}):")
        
        if email_json['attachments']:
            for j, pdf_path in enumerate(email_json['attachments'], 1):
                print(f"  {j}. {pdf_path}")
        else:
    ...
    # Save to JSON file
    json_filename = f"order_emails_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(json_filename, 'w', encoding='utf-8') as f:
        json.dump(emails_json, f, indent=2, ensure_ascii=False)
    
    print(f"\n💾 JSON saved to: {json_filename}")
    print(f"📊 Total order emails: {len(emails_json)}")
    print(f"📎 Total PDF attachments: {sum(len(email['attachments']) for email in emails_json)}")
